...
struct KoreanKeyboardRouter {
    ...
    mutating func handle(code: Int, pressed: Bool, mapBacktick: Bool = false,
        ...
        let modifiers = held.filter { (224...231).contains($0) }
    ...
    mutating func toggle() -> [Event] {
        repeatKey = nil
        // Don't send Ctrl+RightAlt or Shift+RightAlt when translating a chord.
        // Restore the physical modifiers afterwards so subsequent typing works.
        ...
        return modifiers.map { Event(code: $0, pressed: false) }
            ...
            + modifiers.map { Event(code: $0, pressed: true) }
    }
